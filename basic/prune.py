...
class PruningModule(Module):
    def prune_by_percentile(self, q=5.0, **kwargs):
        ...
        # modified here!!!!! Just FC1 and FC2 and fc_loc.
            ...
            if name in ['fc1', 'fc2']:
                ...
                if name == 'stn':
                    for subname, submodule in module.named_modules():
                        if subname in ['fc_loc.0', 'fc_loc.2']:
                           submodule.prune(threshold=percentile_value)
        ...
        for name, module in self.named_modules():
            if name in ['fc1', 'fc2']:
                ...
                if name == 'stn':
                    for subname, submodule in module.named_modules():
                        if subname in ['fc_loc.0', 'fc_loc.2']:
                            threshold = np.std(submodule.weight.data.cpu().numpy()) * s
                            print(f'Pruning with threshold : {threshold} for layer {subname}')
                            submodule.prune(threshold)
